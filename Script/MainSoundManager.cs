...
public class MainSoundManager : MonoBehaviour
{
    ...
    public AudioSource maintargetAudioSource; // Inspector에서 대상 AudioSource를 할당하기 위한 public 변수
    //public Manager manager;

    private void Start()
    {
        //manager.musicVolume = 0.4f;
        // 슬라이더의 값을 PlayerPrefs에서 읽어와 대상 AudioSource의 볼륨으로 설정
        float savedVolume = PlayerPrefs.GetFloat("MainMusicVolume", 1f); // 기본값은 1 (최대 볼륨)
        soundSlider.value = savedVolume;
        maintargetAudioSource.volume = savedVolume;
    ...
    {
        maintargetAudioSource.volume = volume;
    ...
    public void SaveCurrentVolume()
    {
        ...
        PlayerPrefs.SetFloat("MainMusicVolume", maintargetAudioSource.volume);
        PlayerPrefs.Save();
    }
}
